...
This script writes its output to a ephys_features_df.csv file that 
contains the electrophysiological features.
"""
...
for x in annotations["sample_id"]:
    ...
    feature_dict['RS AHP Amp. (mV)'] = spikes_dict['rs']['fast_ahp_amp'].magnitude
    feature_dict['RS Max. Slope (mV/ms)'] = spikes_dict['rs']['spike_max_slope']
    feature_dict['RS Min. Slope (mV/ms)'] = spikes_dict['rs']['spike_min_slope']
    feature_dict['RS Peak (mV)'] = spikes_dict['rs']['spike_peak'].magnitude - ljp,
    feature_dict['RS Half Width (ms)'] = spikes_dict['rs']['half_width'].magnitude,
    feature_dict['RS Threshold (mV)'] = spikes_dict["rs"]["spike_threshold"].magnitude - ljp,
    feature_dict['LS AHP Amp. (mV)'] = spikes_dict['ls']['fast_ahp_amp'].magnitude
    feature_dict['LS Max. Slope (mV/ms)'] = spikes_dict['ls']['spike_max_slope']
    feature_dict['LS Min. Slope (mV/ms)'] = spikes_dict['ls']['spike_min_slope']
    feature_dict['LS Peak (mV)'] = spikes_dict['ls']['spike_peak'].magnitude - ljp,
    feature_dict['LS Half Width (ms)'] = spikes_dict['ls']['half_width'].magnitude,
    feature_dict['LS Threshold (mV)'] = spikes_dict["ls"]["spike_threshold"].magnitude - ljp,
...
fig = plt.figure(figsize=(7.5,13.33),
                 constrained_layout=True)

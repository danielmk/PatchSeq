...
adata_path = os.path.join(dirname, "data",
                          "count_exons_introns_full_named.h5ad")
adata = sc.read_h5ad(adata_path)

adata.obs.index = np.array([x.split(".")[0] for x in adata.obs.index],
                           dtype=str)

alignment_path = os.path.join(dirname, "data", "mapping_stats.csv")
alignment = pd.read_csv(alignment_path, delimiter=";", index_col=0)
...
annotations = pd.read_csv(annotations_path, delimiter=";", index_col=0)
adata.obs = pd.concat([adata.obs, annotations], axis=1, sort=False,
                      join="inner")
...
adata.obs["perc_mito_counts"] = (
    adata.obs["total_mito_counts"] / adata.obs["total_counts"]
) * 100

perc_mito_gene_counts = adata.obs["perc_mito_counts"]
cutoff = 10.0
perc_mito_gene_counts_excl = perc_mito_gene_counts > cutoff
perc_mito_gene_counts_excl_idc = adata.obs.index[perc_mito_gene_counts_excl]

# Perc Aligned
mad = mad_exclusion(adata.obs["perc_of_total_exon"])
exon_alignment_bool, exon_alignment_thr = mad
intron_alignment_bool, intron_alignment_thr = mad_exclusion(
    adata.obs["perc_of_total_intron"]
)
...
all_excluded_samples_bool = (
    total_counts_bool
    | n_genes_bool
    | perc_mito_gene_counts_excl
    | exon_alignment_bool
...
sns.distplot(total_counts_log10, ax=ax[0, 0], norm_hist=False, kde=False,
             bins=20)
ax[0, 0].vlines(total_counts_thr, 0, 45, linestyle="dashed")
sns.distplot(n_genes_log10, ax=ax[0, 1], norm_hist=False, kde=False,
             bins=10)
ax[0, 1].vlines(n_genes_thr, 0, 45, linestyle="dashed")
sns.distplot(perc_mito_gene_counts, ax=ax[0, 2], norm_hist=False, kde=False,
             bins=100)
...
sns.distplot(
    adata.obs["perc_of_total_exon"], ax=ax[1, 0], norm_hist=False, kde=False,
    bins=40
)
ax[1, 0].vlines(exon_alignment_thr, 0, 20, linestyle="dashed")
sns.distplot(
    adata.obs["perc_of_total_intron"], ax=ax[1, 1], norm_hist=False, kde=False,
    bins=40
)
...
sc.pp.normalize_total(
    adata, target_sum=1e6, exclude_highly_expressed=True, inplace=True
)
...
# Plotting gene
sns.distplot(mean_counts_log10, ax=ax[1, 2], norm_hist=False, kde=False,
             bins=40)
...
adata_save_path = os.path.join(
    dirname, "data", "count_exons_introns_full_named_postqc.h5ad"
)

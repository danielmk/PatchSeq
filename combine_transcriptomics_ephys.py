...
trans_path = os.path.join(dirname, 'data', 'count_exons_introns_full_named_postqc.h5ad')
trans_adata = sc.read_h5ad(trans_path)
trans_adata.obs.index = np.array([x.split('.')[0] for x in trans_adata.obs.index], dtype = str)
...
trans_adata.obs['Transcriptomic Type'][sst_pos & slc17a8_pos] = "SST & Slc17a8 RNA Positive"
...
trans_adata.obs = pd.concat([trans_adata.obs, ephys_df[unique_cols]],
                            axis=1,
                            sort=False,
                            join='inner')

unique_cols = trans_adata.obs.columns.difference(ephys_df.columns)
ephys_df = pd.concat([ephys_df, trans_adata.obs[unique_cols]],
                      axis=1,
                      sort=False,
                      join='outer')
